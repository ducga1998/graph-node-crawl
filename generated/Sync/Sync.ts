...
export class Sync__getReservesResult {
  value0: BigInt;
  value1: BigInt;
  value2: BigInt;
...
export class Sync extends ethereum.SmartContract {
  static bind(address: Address): Sync {
    return new Sync("Sync", address);
  }
  ...
  burn(to: Address): Sync__burnResult {
    let result = super.call("burn", "burn(address):(uint256,uint256)", [
      ethereum.Value.fromAddress(to)
    ]);

    return new Sync__burnResult(result[0].toBigInt(), result[1].toBigInt());
  }

  try_burn(to: Address): ethereum.CallResult<Sync__burnResult> {
    ...
    return ethereum.CallResult.fromValue(
      new Sync__burnResult(value[0].toBigInt(), value[1].toBigInt())
    );
  }
  ...
  getReserves(): Sync__getReservesResult {
    let result = super.call(
      "getReserves",
    ...
    return new Sync__getReservesResult(
      result[0].toBigInt(),
      result[1].toBigInt(),
      result[2].toBigInt()
    );
  ...
  try_getReserves(): ethereum.CallResult<Sync__getReservesResult> {
    let result = super.tryCall(
      "getReserves",
      "getReserves():(uint112,uint112,uint32)",
      []
